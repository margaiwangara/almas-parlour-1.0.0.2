<?php

//include master file
include_once 'include/master.inc.php';

?>
<title>Alma's Parlour - Contact Us</title>
<div class="row">
    <div class="col-md-12 ">
        <div class="col-md-4">
        <div class="well" style="font-family: Calibri;">
            <table class="table table-condensed" >
                <tbody>
                <tr>
                    <th>E-Mail</th>
                    <td>someone@example.com</td>
                </tr>
                <tr>
                    <th>Telephone</th>
                    <td>05547896534</td>
                </tr>
                <tr>
                    <td></td>
                    <td>04526358975</td>
                </tr>
                </tbody>
            </table>
            <table class="table table-condensed">
                <tbody>
                <tr>
                    <th>Address</th>
                </tr>
                <tr>
                    <td>123 Conway Drive, Mulholand, United States</td>
                </tr>
                </tbody>
            </table>
            <table class="table table-condensed">
                <tbody>
                <tr>
                    <th>Follow Us</th>
                </tr>
                <tr>
                    <td><span class="fa fa-facebook-square"></span> <a href="#">Facebook</a></td>
                </tr>
                <tr>
                    <td><span class="fa fa-twitter"></span> <a href="#">Twitter</a></td>
                </tr>
                <tr>
                    <td><span class="fa fa-linkedin"></span> <a href="#">Linked In</a></td>
                </tr>
                <tr>
                    <td><span class="fa fa-google-plus-square"></span> <a href="#">Google+</a></td>
                </tr>
                </tbody>
            </table>
        </div>
        </div>
        <div class="col-md-6">
            <div class="well">
                <form action="send-messages.php" method="get">
                    <div class="form-group">
                        <input type="text" class="form-control" name="user_name" id="user_name_id" placeholder="Name"/>
                    </div>
                    <div class="form-group">
                        <input type="email" class="form-control" name="user_email" id="user_email_id" placeholder="user@example.com" <?php if(isset($_SESSION['USER_EMAIL'])) echo "value='".$_SESSION['USER_EMAIL']."' readonly";?> required/>
                    </div>
                    <div class="form-group">
                        <textarea class="form-control" rows="9" name="user_message" id="user_message_id" placeholder="Write Message..." style="resize: none;" required></textarea>
                    </div>
                    <div class="form-group">
                        <button type="submit" class="btn btn-info">
                            Send Message
                        </button>
                    </div>
                </form>
            </div>
        </div>
    </div>
</div>